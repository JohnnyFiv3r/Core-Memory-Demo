...
type GraphNode = {
  ...
  size: number
  fill: string
  data: { bead_id: string; title?: string; type?: string; status?: string; source_turn_ids: string[] }
}
...
type GraphEdge = {
  ...
  label: string
  size?: number
  data?: { relationship?: string; confidence?: number; reason_text?: string }
}
...
function App(): React.JSX.Element {
  ...
  const graphData = useMemo(() => {
    ...
    const edges: GraphEdge[] = filteredEdges.map((e, idx) => ({
      id: String(e.id || `${e.source || 'source'}-${e.target || 'target'}-${idx}`),
