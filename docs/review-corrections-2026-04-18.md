...
- Semantic freshness behavior update: chat turns now run an immediate semantic-only async-jobs pass before retrieval diagnostics (`demo_chat_sync_semantic_each_turn=true` by default), so mutated beads are embedded per turn instead of waiting for the periodic background tick.
